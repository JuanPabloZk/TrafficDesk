...
// All data is loaded dynamically from Supabase / Meta API
const clients=[];
const metaRows=[];
const googleRows=[];
...
export default function Dashboard(){
  ...
  // Client budgets/settings keyed by accountId
  ...
  useEffect(()=>{
    ...
    const loadAll=async()=>{
      ...
      if(aData)setAlerts(aData.map(a=>({id:a.id,sev:a.severity,client:a.client||"",msg:a.message||"",age:timeAgo(a.created_at),plat:a.platform||"meta",resolved:false})));
      ...
      // Client settings (budget, notes)
      const{data:csData}=await supabase.from("client_settings").select("*").eq("user_id",user.id);
      if(csData){
        const map={};
        csData.forEach(s=>{map[`${s.platform}_${s.account_id}`]=s;});
        setClientSettings(map);
      }

      // User preferences
      const{data:pData}=await supabase.from("user_preferences").select("*").eq("user_id",user.id).single();
      if(pData){
        if(pData.dash_period)setDashPeriod(pData.dash_period);
        if(pData.rep_period)setRepPeriod(pData.rep_period);
      }
      setPrefsLoaded(true);

      // Metrics cache — load last cached data to show immediately
      const{data:mcData}=await supabase.from("metrics_cache").select("*").eq("user_id",user.id);
      if(mcData&&mcData.length>0){
        const cache={};
        mcData.forEach(m=>{cache[`${m.account_id}_${m.period}`]=m.data;});
        setMetricsCache(cache);
        // Use cached metrics as initial live data if fresh (< 2h old)
        const fresh=mcData.filter(m=>new Date()-new Date(m.fetched_at)<7200000);
        if(fresh.length>0){
          const merged=fresh.map(m=>m.data).flat();
          if(merged.length>0)setLiveMetrics(merged);
        }
      }
    };
    loadAll();
  ...
  },[dashPeriod,user?.id,prefsLoaded]);

  useEffect(()=>{
    if(!user?.id||!prefsLoaded)return;
    supabase.from("user_preferences").upsert({user_id:user.id,last_view:view,updated_at:new Date().toISOString()},{onConflict:"user_id"});
  },[view,user?.id,prefsLoaded]);
  ...
  const generateAlerts=async(metrics)=>{
    ...
    for(const m of metrics){
      if(parseFloat(m.ctr)<1){
        newAlerts.push({user_id:user.id,severity:"med",client:m.client,message:`CTR abaixo de 1% (${m.ctr}%) — verifique os criativos`,platform:"meta",auto_generated:true,metric_value:parseFloat(m.ctr),threshold:1});
      }
      const roas=parseFloat(m.investido)>0?parseFloat(m.valorResult)/parseFloat(m.investido):0;
      if(roas>0&&roas<2){
        newAlerts.push({user_id:user.id,severity:"high",client:m.client,message:`ROAS baixo: ${roas.toFixed(2)}x — abaixo de 2x, revise a segmentação`,platform:"meta",auto_generated:true,metric_value:roas,threshold:2});
      }
      if(parseFloat(m.investido)===0&&m.impressoes===0){
        newAlerts.push({user_id:user.id,severity:"high",client:m.client,message:`Sem dados de investimento — verifique se as campanhas estão ativas`,platform:"meta",auto_generated:true});
      }
    }
    if(newAlerts.length>0){
      // Remove old auto-generated alerts first
      await supabase.from("alerts").delete().eq("user_id",user.id).eq("auto_generated",true);
      const{data}=await supabase.from("alerts").insert(newAlerts).select();
      if(data)setAlerts(data.map(a=>({id:a.id,sev:a.severity,client:a.client||"",msg:a.message||"",age:"agora",plat:a.platform||"meta",resolved:false})));
    }
  };

  // ── Save client budget ─────────────────────────────────────
  const saveClientBudget=async(accountId,platform,budget)=>{
    if(!user?.id)return;
  ...
  // ── Save metrics to cache after fetch ─────────────────────
  const cacheMetrics=async(metrics,period)=>{
    if(!user?.id||!metrics)return;
    await Promise.all(metrics.map(m=>
      supabase.from("metrics_cache").upsert({
        user_id:user.id,account_id:m.accountId,platform:"meta",period,
        data:m,fetched_at:new Date().toISOString(),
      },{onConflict:"user_id,account_id,platform,period"})
    ));
  };
  ...
  const[waTab,setWaTab]=useState("campaigns");
  ...
  // Load budget rules from Supabase
  useEffect(()=>{
    if(!user?.id)return;
    const load=async()=>{
      setBudgetLoading(true);
      const{data}=await supabase.from("budget_rules").select("*").eq("user_id",user.id).order("created_at",{ascending:false});
      if(data)setBudgetRules(data);
      setBudgetLoading(false);
    };
    load();
  },[user?.id]);

  // Check budgets against live metrics whenever metrics update
  useEffect(()=>{
  ...
  const deleteBudgetRule=async(id)=>{
    ...
    await supabase.from("budget_rules").delete().eq("id",id).eq("user_id",user.id);
  };

  const updateBudgetRule=async(id,field,value)=>{
    setBudgetRules(prev=>prev.map(r=>r.id===id?{...r,[field]:value}:r));
    await supabase.from("budget_rules").update({[field]:value,updated_at:new Date().toISOString()}).eq("id",id).eq("user_id",user.id);
  };
  ...
  const createCampaign=async()=>{
    ...
    const newRow={
      user_id:user.id,
      name:newCamp.name,client:newCamp.client,
      status:"draft",total,sent:0,delivered:0,
      read_count:0,replied:0,converted:0,
      msg:newCamp.msg,
      scheduled_at:newCamp.scheduledAt||null,
    };
    const{data,error}=await supabase.from("wa_campaigns").insert(newRow).select().single();
    if(!error&&data){
      setWaCampaigns(cs=>[{id:data.id,name:data.name,client:data.client,status:data.status,total:data.total,sent:0,delivered:0,read:0,replied:0,converted:0,createdAt:now,scheduledAt:data.scheduled_at||"",msg:data.msg},...cs]);
    }
  ...
  const sendCampaign=async(id)=>{
    ...
    await supabase.from("wa_campaigns").update({status:"sending"}).eq("id",id).eq("user_id",user.id);
  };
  ...
  const periods=[
    ...
    {v:"yesterday",l:"Ontem"},
    {v:"last_7d",l:"7 dias"},
    {v:"last_14d",l:"14 dias"},
    {v:"last_30d",l:"30 dias"},
    {v:"last_90d",l:"90 dias"},
  ...
  const saveConnection=async(platform,token,bmId,accounts)=>{
    if(!user?.id)return;
    ...
    // Save accounts list in localStorage as cache
    localStorage.setItem(`td_${platform}_accounts`,JSON.stringify(accounts));
  };

  const clearConnection=async(platform)=>{
    if(!user?.id)return;
    await supabase.from("connections").upsert({
      user_id:user.id, platform, status:"idle",
      access_token:null, bm_id:null,
      updated_at:new Date().toISOString(),
    },{onConflict:"user_id,platform"});
    ...
    const loadConnections=async()=>{
      try{
        const{data}=await supabase
          ...
          .eq("user_id",user.id)
          .eq("status","connected");
        if(!data||data.length===0){setRestoringConn(false);return;}
        ...
        for(const conn of data){
          if(conn.platform==="meta"&&conn.access_token&&conn.bm_id){
            ...
            if(cached){
              ...
              }catch(e){console.error("Auto-restore meta failed:",e);}
            }
          }
        }
      }finally{setRestoringConn(false);}
    };
    loadConnections();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[user?.id]);
  ...
  const resolveAlert=async(id)=>{
    ...
    await supabase.from("alerts").update({resolved:true}).eq("id",id).eq("user_id",user.id);
  };
  ...
  const addTask=async()=>{
    ...
    const newRow={user_id:user.id,title:nt.title,client:nt.client,status:"todo",priority:nt.priority,due:"–"};
    const{data,error}=await supabase.from("tasks").insert(newRow).select().single();
    if(!error&&data){
      setTasks(ts=>[{id:data.id,title:data.title,client:data.client,status:data.status,priority:data.priority,due:data.due||"–"},...ts]);
    }
      ...
      <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        <div style={{height:isMobile?52:58,padding:`0 ${isMobile?14:28}px`,borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",background:T.side,flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            {restoringConn&&<div style={{fontSize:11,color:T.mute,display:"flex",alignItems:"center",gap:5}}><span style={{width:10,height:10,border:"1.5px solid rgba(99,102,241,.3)",borderTop:"1.5px solid #6366F1",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}></span>Restaurando...</div>}
            {!isMobile&&!restoringConn&&<div style={{fontSize:11,color:T.mute,background:"rgba(255,255,255,.04)",border:`1px solid ${T.border}`,borderRadius:8,padding:"5px 12px"}}>{new Date().toLocaleDateString("pt-BR",{day:"2-digit",month:"short",year:"numeric"})}</div>}
          </div>
        </div>
        ...
        <div style={{padding:P,flex:1}}>
            ...
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
                <div style={{display:"flex",gap:3,background:"rgba(255,255,255,.04)",borderRadius:9,padding:3,border:`1px solid ${T.border}`,flexWrap:"wrap"}}>
                  {periods.map(p=>(
                    <button key={p.v} onClick={()=>changePeriod(p.v)} style={{
                      padding:"5px 12px",borderRadius:7,border:"none",cursor:"pointer",
                      fontFamily:T.font,fontSize:11,fontWeight:dashPeriod===p.v?600:400,
                      background:dashPeriod===p.v?"rgba(99,102,241,.15)":"transparent",
                      color:dashPeriod===p.v?T.accent:T.sub,transition:"all .12s",
                    }}>{p.l}</button>
                  ))}
                </div>
                {metaStatus==="connected"&&(
                  <button onClick={()=>fetchInsights(metaAccounts,metaForm.accessToken,dashPeriod)}
                    ...
                    {fetchingInsights?"Atualizando...":"Atualizar"}
                  </button>
                )}
                <div style={{marginLeft:"auto",fontSize:11,color:T.mute,display:"flex",alignItems:"center",gap:5}}>
                  {metaStatus==="connected"&&liveMetrics&&!fetchingInsights&&(
                    <span style={{color:T.ok,display:"flex",alignItems:"center",gap:4}}>
                      <span className="sdot" style={{background:T.ok}}></span>
                      Dados reais · {periods.find(p=>p.v===dashPeriod)?.l}
                    </span>
                  )}
                  {metaStatus!=="connected"&&<span style={{color:T.mute}}>Conecte o BM para ver dados reais</span>}
                </div>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:isMobile?10:12,marginBottom:isMobile?14:18}}>
                ...
                <MCard label="ROAS Meta" value={avgMROAS+"x"} sub={liveMetrics?"dados reais":"sem dados"} accent={T.meta} m={isMobile}/>
              ...
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 360px",gap:12,marginBottom:12}}>
                ...
                <div className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"18px 20px",display:"flex",flexDirection:"column"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    {alerts.length===0&&(
                      <div style={{textAlign:"center",padding:"24px 0",fontSize:12,color:T.mute}}>
                        <div style={{fontSize:22,marginBottom:8}}>✅</div>
                        {metaStatus==="connected"?"Nenhum alerta ativo":"Conecte o BM para gerar alertas"}
                      </div>
                    )}
                    {alerts.map(a=>(
                      <div key={a.id} style={{background:sevBg[a.sev],borderRadius:10,border:`1px solid rgba(255,255,255,.04)`,borderLeft:`3px solid ${sevC[a.sev]}`,padding:"12px 14px"}}>
                        ...
                        <button onClick={()=>resolveAlert(a.id)} style={{marginTop:6,fontSize:9,color:T.mute,background:"rgba(255,255,255,.05)",border:`1px solid ${T.border}`,borderRadius:5,padding:"2px 8px",cursor:"pointer",fontFamily:T.font}}>✓ Resolver</button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {view==="clients"&&(
            <div>
              ...
              {activeClients.map(c=>{
                ...
                const cs=clientSettings[`meta_${c.accountId}`];
                ...
                return(
                  <div key={c.id} className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"20px 22px"}}>
                    ...
                    {/* Budget field — saves to Supabase on blur */}
                    <div style={{marginBottom:10}}>
                      <div style={{fontSize:9,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>Orçamento mensal</div>
                      <input
                        type="number"
                        defaultValue={budget||""}
                        placeholder="R$ 0,00"
                        onBlur={e=>saveClientBudget(c.accountId,"meta",parseFloat(e.target.value)||0)}
                        style={{width:"100%",background:"rgba(255,255,255,.04)",border:`1px solid ${T.border}`,borderRadius:8,padding:"8px 12px",color:T.txt,fontSize:12,fontFamily:T.mono,boxSizing:"border-box"}}
                      />
                    </div>
                    {budget>0&&(
                      <div>
                    ...
                    <div style={{marginTop:8}}>
                      <span style={{fontSize:10,fontFamily:T.mono,color:T.mute}}>ID: {c.accountId||"–"}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {view==="tasks"&&(
            <div>
              {tasksLoading?(
                <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"60px 0",gap:10,color:T.mute,fontSize:13}}>
                  <span style={{width:16,height:16,border:"2px solid rgba(99,102,241,.2)",borderTop:`2px solid ${T.accent}`,borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}></span>
                  Carregando tarefas...
                </div>
              ):(
          ...
          {/* ═══════ REPORTS ═══════ */}
          {view==="reports"&&(()=>{
            // ── Build metrics from liveMetrics (real Meta API data) ──
            const zero={impressoes:0,alcance:0,ctr:0,cpc:0,investido:0,conversoes:0,valorResult:0};

            const getMetrics=(clientName)=>{
              if(!liveMetrics||liveMetrics.length===0)return zero;
              if(clientName==="all"){
                const agg=liveMetrics.reduce((acc,d)=>({
                  impressoes:acc.impressoes+d.impressoes,
                  alcance:acc.alcance+d.alcance,
                  investido:acc.investido+parseFloat(d.investido),
                  conversoes:acc.conversoes+d.conversoes,
                  valorResult:acc.valorResult+parseFloat(d.valorResult),
                  ctr:0,cpc:0,
                }),{...zero});
                const n=liveMetrics.length;
                agg.ctr=parseFloat((liveMetrics.reduce((s,d)=>s+parseFloat(d.ctr),0)/n).toFixed(2));
                agg.cpc=parseFloat((liveMetrics.reduce((s,d)=>s+parseFloat(d.cpc),0)/n).toFixed(2));
                return agg;
              }
              const d=liveMetrics.find(r=>r.client===clientName||r.accountId===clientName);
              if(!d)return zero;
              return{
                ...
                investido:parseFloat(d.investido),
                conversoes:d.conversoes,
                valorResult:parseFloat(d.valorResult),
              };
            };
            ...
            const fetchRepData=async()=>{
              if(metaStatus!=="connected"||metaAccounts.length===0){
                setRepError("Conecte o Business Manager na aba Conexões para ver os dados reais.");
                return;
              }
              ...
              try{
                ...
                if(accs.length===0){setRepError("Nenhuma conta encontrada para este filtro.");setRepLoading(false);return;}
                const results=await Promise.all(accs.map(async(acc)=>{
                  const res=await fetch(
                    `${GRAPH}/act_${acc.id}/insights`+
                    `?fields=impressions,reach,clicks,ctr,cpc,spend,actions,action_values`+
                    `&date_preset=${repPeriodMap[repPeriod]||"last_30d"}&level=account`+
                ...
                const valid=results.filter(Boolean);
                setRepData(valid);
              }catch(e){setRepError("Erro ao buscar dados: "+e.message);}
              finally{setRepLoading(false);}
            };

            // Aggregate repData for display
            const rd=repData;
            const agg=rd&&rd.length>0?rd.reduce((acc,d)=>({
              impressoes:acc.impressoes+d.impressoes,
              alcance:acc.alcance+d.alcance,
              cliques:acc.cliques+(d.cliques||0),
              ctr:0,cpc:0,
              investido:acc.investido+parseFloat(d.investido),
              conversoes:acc.conversoes+d.conversoes,
              valorResult:acc.valorResult+parseFloat(d.valorResult),
            }),{impressoes:0,alcance:0,cliques:0,ctr:0,cpc:0,investido:0,conversoes:0,valorResult:0}):null;
            if(agg&&rd.length>0){
              agg.ctr=parseFloat((rd.reduce((s,d)=>s+parseFloat(d.ctr),0)/rd.length).toFixed(2));
              agg.cpc=parseFloat((rd.reduce((s,d)=>s+parseFloat(d.cpc),0)/rd.length).toFixed(2));
            }
            ...
            const exportCSV=()=>{
              ...
              const rows=[
                ["Métrica","Valor"],
                ["Cliente",repClient==="all"?"Todas as contas":repClient],
                ["Período",repPeriod==="7d"?"7 dias":repPeriod==="30d"?"30 dias":"90 dias"],
                ["Impressões",agg.impressoes.toLocaleString("pt-BR")],
                ["Alcance",agg.alcance.toLocaleString("pt-BR")],
                ["Cliques",agg.cliques.toLocaleString("pt-BR")],
                ["CTR",agg.ctr+"%"],
                ["CPC","R$ "+agg.cpc],
                ["Valor Investido","R$ "+agg.investido.toLocaleString("pt-BR",{minimumFractionDigits:2})],
                ["Conversões",agg.conversoes],
                ["Valor do Resultado","R$ "+agg.valorResult.toLocaleString("pt-BR",{minimumFractionDigits:2})],
                ["ROAS",roas+"x"],
                ["CPL","R$ "+cpl],
                [],
                ["DETALHAMENTO POR CONTA"],
                ["Conta","Impressões","Alcance","CTR","CPC","Investido","Conversões","Resultado"],
                ...(rd||[]).map(d=>[d.client,d.impressoes,d.alcance,d.ctr+"%","R$"+d.cpc,"R$"+d.investido,d.conversoes,"R$"+d.valorResult]),
              ];
              const a=document.createElement("a");
              a.href=URL.createObjectURL(new Blob(["\uFEFF"+rows.map(r=>r.join(",")).join("\n")],{type:"text/csv;charset=utf-8;"}));
              a.download=`relatorio_${repPeriod}_${Date.now()}.csv`;
              a.click();
            };

            const MetBox=({label,value,sub,accent})=>(
              <div className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:isMobile?"14px 16px":"18px 20px",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{fontFamily:T.mono,fontSize:isMobile?20:26,color:T.txt,fontWeight:500,letterSpacing:"-0.01em",marginBottom:4}}>{value}</div>
            ...
            return(
              <div>
                {/* ── Selector bar ── */}
                <div style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"16px 20px",marginBottom:16,display:"flex",gap:10,alignItems:isMobile?"flex-start":"center",flexWrap:"wrap",flexDirection:isMobile?"column":"row"}}>
                  {/* Account selector */}
                  <div style={{flex:1,minWidth:160}}>
                    <div style={{fontSize:9,color:T.mute,letterSpacing:".09em",fontWeight:500,marginBottom:5}}>CONTA</div>
                    <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                      {[{v:"all",l:"Todas"},...(metaAccounts.map(a=>({v:a.name,l:a.name.split(" ").slice(0,2).join(" ")})))].map(opt=>(
                        <button key={opt.v} onClick={()=>setRepClient(opt.v)} style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${repClient===opt.v?T.meta+"88":T.border}`,background:repClient===opt.v?"rgba(0,129,251,.1)":"transparent",color:repClient===opt.v?T.meta:T.sub,fontSize:11,fontFamily:T.font,cursor:"pointer"}}>
                          {opt.l}
                        </button>
                      ))}
                  ...
                  <div>
                    ...
                    <div style={{display:"flex",gap:4}}>
                      {[{v:"7d",l:"7 dias"},{v:"30d",l:"30 dias"},{v:"90d",l:"90 dias"}].map(p=>(
                        <button key={p.v} onClick={()=>setRepPeriod(p.v)} style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${repPeriod===p.v?"#60a5fa88":T.border}`,background:repPeriod===p.v?"rgba(96,165,250,.1)":"transparent",color:repPeriod===p.v?"#60a5fa":T.sub,fontSize:11,fontFamily:T.font,cursor:"pointer"}}>
                          {p.l}
                        </button>
                      ))}
                    </div>
                  ...
                  <button className="btn-p" onClick={fetchRepData} disabled={repLoading} style={{padding:"9px 18px",borderRadius:9,border:"none",background:T.meta,color:"#fff",cursor:repLoading?"not-allowed":"pointer",fontSize:12,fontWeight:600,fontFamily:T.font,display:"flex",alignItems:"center",gap:7,flexShrink:0,opacity:repLoading?.7:1}}>
                    {repLoading
                      ?<><span style={{width:12,height:12,border:"2px solid rgba(255,255,255,.3)",borderTop:"2px solid #fff",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}></span>Buscando...</>
                      :<><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M10 6A4 4 0 1 1 6 2a4 4 0 0 1 2.83 1.17L10 4" stroke="white" strokeWidth="1.4" strokeLinecap="round"/><path d="M10 1v3H7" stroke="white" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>Buscar dados</>
                    }
                  </button>

                  {agg&&<button className="btn-p" onClick={exportCSV} style={{padding:"9px 16px",borderRadius:9,border:"none",background:T.accent,color:"#fff",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:T.font,display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
                    <IcoDl/>Exportar CSV
                  </button>}
                </div>

                {/* ── Error / empty state ── */}
                {repError&&<div style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",borderRadius:12,padding:"14px 18px",fontSize:12,color:T.err,marginBottom:16}}>⚠ {repError}</div>}

                {!agg&&!repLoading&&!repError&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"60px 20px",textAlign:"center"}}>
                    <div style={{fontSize:40,marginBottom:16}}>📊</div>
                    <div style={{fontSize:15,fontWeight:600,color:T.txt,marginBottom:8}}>Selecione a conta e o período</div>
                    <div style={{fontSize:13,color:T.mute,maxWidth:340,lineHeight:1.7,marginBottom:20}}>Escolha uma conta e o período acima e clique em <strong style={{color:T.txt}}>Buscar dados</strong> para ver as métricas reais da Meta API.</div>
                    {metaStatus!=="connected"&&(
                      <button onClick={()=>setView("connections")} className="btn-p" style={{padding:"10px 22px",borderRadius:10,border:"none",background:T.accent,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:T.font}}>
                ...
                {agg&&(
                  <div>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
                      <div style={{width:38,height:38,borderRadius:10,background:"rgba(0,129,251,.1)",border:"1px solid rgba(0,129,251,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:T.meta}}>
                        {repClient==="all"?"ALL":repClient.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase()}
                      </div>
                      <div>
                        <div style={{fontSize:14,fontWeight:600,color:T.txt}}>{repClient==="all"?"Todas as contas":repClient}</div>
                        <div style={{fontSize:11,color:T.mute}}>{repPeriod==="7d"?"Últimos 7 dias":repPeriod==="30d"?"Últimos 30 dias":"Últimos 90 dias"} · Meta Ads · {rd.length} conta(s)</div>
                      </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:12,marginBottom:12}}>
                      <MetBox label="Impressões" value={agg.impressoes.toLocaleString("pt-BR")} sub="total no período" accent="#a78bfa"/>
                      <MetBox label="Alcance" value={agg.alcance.toLocaleString("pt-BR")} sub="pessoas únicas" accent="#60a5fa"/>
                      <MetBox label="CTR" value={agg.ctr+"%"} sub={parseFloat(agg.ctr)<1?"abaixo da média":"bom"} accent={parseFloat(agg.ctr)<1?T.warn:T.ok}/>
                      <MetBox label="CPC" value={"R$ "+parseFloat(agg.cpc).toLocaleString("pt-BR",{minimumFractionDigits:2})} sub="custo por clique" accent={T.sub}/>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)",gap:12,marginBottom:16}}>
                      <MetBox label="Valor Investido" value={"R$ "+agg.investido.toLocaleString("pt-BR",{minimumFractionDigits:2})} sub={repPeriod==="7d"?"última semana":repPeriod==="30d"?"último mês":"último trimestre"} accent={T.accent}/>
                      <MetBox label="Conversões" value={agg.conversoes.toLocaleString("pt-BR")} sub={`CPL: R$ ${cpl}`} accent={T.ok}/>
                      <MetBox label="Valor do Resultado" value={"R$ "+agg.valorResult.toLocaleString("pt-BR",{minimumFractionDigits:2})} sub={`ROAS: ${roas}x`} accent={T.err}/>
                    </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12,marginBottom:16}}>
                      <div className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"18px 22px",display:"flex",alignItems:"center",gap:16}}>
                        <div style={{width:52,height:52,borderRadius:14,background:parseFloat(roas)>=3?"rgba(34,197,94,.1)":"rgba(245,158,11,.1)",border:`1px solid ${parseFloat(roas)>=3?"rgba(34,197,94,.2)":"rgba(245,158,11,.2)"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                          <span style={{fontFamily:T.mono,fontSize:16,fontWeight:700,color:parseFloat(roas)>=3?T.ok:T.warn}}>{roas}x</span>
                        </div>
                        <div>
                          ...
                          <div style={{fontSize:12,color:T.sub,lineHeight:1.6}}>Para cada <span style={{color:T.txt,fontWeight:500}}>R$ 1,00</span> investido, gerou <span style={{color:parseFloat(roas)>=3?T.ok:T.warn,fontWeight:600}}>R$ {roas}</span> em resultado.</div>
                          <div style={{marginTop:8}}>
                      ...
                      <div className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"18px 22px",display:"flex",alignItems:"center",gap:16}}>
                        ...
                        <div>
                          <div style={{fontSize:10,color:T.mute,letterSpacing:".09em",textTransform:"uppercase",marginBottom:4}}>CPL — Custo por conversão</div>
                          <div style={{fontSize:12,color:T.sub,lineHeight:1.6}}>Cada conversão custou em média <span style={{color:"#60a5fa",fontWeight:600}}>R$ {cpl}</span> com <span style={{color:T.txt,fontWeight:500}}>{agg.conversoes} conversões</span>.</div>
                        </div>
                      </div>
                    </div>
                    ...
                    {rd.length>1&&(
                      <div className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"18px 22px"}}>
                        <div style={{fontSize:10,color:T.mute,letterSpacing:".09em",fontWeight:500,marginBottom:14}}>DETALHAMENTO POR CONTA</div>
                        <div style={{overflowX:"auto"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:600}}>
                            <thead><tr>
                              {["Conta","Impressões","Alcance","CTR","CPC","Investido","Conversões","ROAS"].map(h=>(
                                <th key={h} style={{textAlign:"left",padding:"0 0 10px",color:T.mute,fontWeight:500,fontSize:10,letterSpacing:".07em",textTransform:"uppercase",paddingRight:12}}>{h}</th>
                              ))}
                            </tr></thead>
                            <tbody>
                              {rd.map((d,i)=>{
                                const acRoas=parseFloat(d.investido)>0?(parseFloat(d.valorResult)/parseFloat(d.investido)).toFixed(2):"0.00";
                                return(
                                  <tr key={i} className="rh" style={{borderTop:`1px solid ${T.border}`}}>
                                    <td style={{padding:"10px 12px 10px 0",fontWeight:500,color:T.txt}}>{d.client}</td>
                                    <td style={{padding:"10px 12px 10px 0",fontFamily:T.mono,color:T.sub}}>{d.impressoes.toLocaleString("pt-BR")}</td>
                                    <td style={{padding:"10px 12px 10px 0",fontFamily:T.mono,color:T.sub}}>{d.alcance.toLocaleString("pt-BR")}</td>
                                    <td style={{padding:"10px 12px 10px 0",fontFamily:T.mono,color:parseFloat(d.ctr)<1?T.warn:T.ok}}>{d.ctr}%</td>
                                    <td style={{padding:"10px 12px 10px 0",fontFamily:T.mono,color:T.sub}}>R${d.cpc}</td>
                                    <td style={{padding:"10px 12px 10px 0",fontFamily:T.mono,color:T.accent}}>R${parseFloat(d.investido).toLocaleString("pt-BR",{minimumFractionDigits:2})}</td>
                                    <td style={{padding:"10px 12px 10px 0",fontFamily:T.mono,color:T.ok}}>{d.conversoes}</td>
                                    <td style={{padding:"10px 0"}}><span style={{fontFamily:T.mono,fontSize:12,background:parseFloat(acRoas)<3?"rgba(245,158,11,.1)":"rgba(34,197,94,.1)",color:parseFloat(acRoas)<3?T.warn:T.ok,borderRadius:5,padding:"2px 8px"}}>{acRoas}x</span></td>
                                  </tr>
                                );
                              })}
                            </tbody>
          ...
          {/* ═══════ BUDGET CONTROL ═══════ */}
          {view==="budget"&&(()=>{
            const EURO="€"; const REAL="R$"; const USD="$";
            const symb=(c)=>c==="EUR"?EURO:c==="USD"?USD:REAL;
            const fmtMoney=(v,c)=>`${symb(c)} ${parseFloat(v).toLocaleString("pt-BR",{minimumFractionDigits:2})}`;
            const barColor=(pct,alertPct)=>pct>=100?T.err:pct>=(alertPct||80)?T.warn:T.ok;
            const periodLabel={monthly:"Mensal",weekly:"Semanal",daily:"Diário"};

            // Get spent for a rule from live metrics
            const getSpent=(rule)=>{
              if(!liveMetrics)return 0;
              if(rule.account_id){
                const m=liveMetrics.find(d=>d.accountId===rule.account_id);
                return m?parseFloat(m.investido):0;
              }
              return liveMetrics.reduce((s,m)=>s+parseFloat(m.investido),0);
            };
            ...
            return(
              <div>
                ...
                {budgetAlerts.length>0&&(
                  <div style={{background:"rgba(245,158,11,.08)",border:"1px solid rgba(245,158,11,.25)",borderRadius:12,padding:"14px 18px",marginBottom:18}}>
                    <div style={{fontSize:12,fontWeight:600,color:T.warn,marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
                      <span style={{fontSize:16}}>⚠</span> {budgetAlerts.length} regra{budgetAlerts.length>1?"s":""} de verba atingiu{budgetAlerts.length>1?"ram":""} o limite de alerta
                    </div>
                    {budgetAlerts.map(a=>(
                      <div key={a.id} style={{fontSize:12,color:T.sub,lineHeight:1.7}}>
                        • <span style={{color:T.txt,fontWeight:500}}>{a.name}</span>: gastou {fmtMoney(a.spent,a.currency)} de {fmtMoney(a.limit_value,a.currency)} — <span style={{color:a.pct>=100?T.err:T.warn,fontWeight:600}}>{a.pct}% utilizado</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:10}}>
                  <div>
                    ...
                    <div style={{fontSize:12,color:T.mute}}>Defina limites e receba avisos automáticos. Nenhuma alteração é feita nas contas.</div>
                  </div>
                  <button className="btn-p" onClick={()=>setShowBudgetForm(!showBudgetForm)} style={{padding:"9px 18px",borderRadius:9,border:"none",background:T.accent,color:"#fff",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:T.font}}>
                    + Nova Regra
                  </button>
                </div>
                ...
                {/* New rule form */}
                {showBudgetForm&&(
                  <div className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${T.borderMid}`,padding:"20px 22px",marginBottom:18}}>
                    <div style={{fontSize:13,fontWeight:600,color:T.txt,marginBottom:16}}>Nova Regra de Verba</div>
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12,marginBottom:12}}>
                      <div>
                        <div style={{fontSize:10,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>Nome da Regra</div>
                        <input value={newBudget.name} onChange={e=>setNewBudget({...newBudget,name:e.target.value})} placeholder="Ex: Limite mensal — Google Ads" style={{width:"100%",background:"rgba(255,255,255,.04)",border:`1px solid ${T.borderMid}`,borderRadius:8,padding:"9px 12px",color:T.txt,fontSize:12,fontFamily:T.font,boxSizing:"border-box"}}/>
                      </div>
                      <div>
                        <div style={{fontSize:10,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>Conta (opcional)</div>
                      ...
                      <div>
                        <div style={{fontSize:10,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>Valor limite</div>
                        <div style={{display:"flex",gap:6}}>
                          <select value={newBudget.currency} onChange={e=>setNewBudget({...newBudget,currency:e.target.value})} style={{background:"rgba(255,255,255,.04)",border:`1px solid ${T.borderMid}`,borderRadius:8,padding:"9px 10px",color:T.txt,fontSize:12,fontFamily:T.font,flexShrink:0}}>
                      ...
                      <div>
                        <div style={{fontSize:10,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>Período</div>
                    ...
                    <div style={{marginBottom:16}}>
                      <div style={{fontSize:10,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:8}}>Alertar quando atingir: <span style={{color:T.warn,fontFamily:T.mono,fontWeight:600}}>{newBudget.alertPct}%</span></div>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:T.mute,marginTop:4}}>
                        <span>50%</span><span>70%</span><span>80%</span><span>90%</span><span>95%</span>
                      </div>
                    </div>
                    <div style={{display:"flex",gap:8}}>
                      ...
                      <button className="btn-p" onClick={saveBudgetRule} style={{flex:2,padding:"10px 0",borderRadius:9,border:"none",background:T.accent,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:T.font,opacity:(!newBudget.name||!newBudget.limit)?.45:1}}>
                        Salvar Regra
                      </button>
                    </div>
                  </div>
                ...
                {!budgetLoading&&budgetRules.length===0&&!showBudgetForm&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"60px 20px",textAlign:"center"}}>
                    <div style={{width:60,height:60,borderRadius:16,background:"rgba(99,102,241,.1)",border:`1px solid rgba(99,102,241,.2)`,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16,fontSize:28}}>💰</div>
                    <div style={{fontSize:16,fontWeight:600,color:T.txt,marginBottom:8}}>Nenhuma regra de verba</div>
                    <div style={{fontSize:13,color:T.mute,maxWidth:340,lineHeight:1.7,marginBottom:20}}>Crie regras para monitorar o gasto de cada conta e receba avisos antes de ultrapassar o limite.</div>
                    <button className="btn-p" onClick={()=>setShowBudgetForm(true)} style={{padding:"10px 22px",borderRadius:10,border:"none",background:T.accent,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:T.font}}>
                ...
                {!budgetLoading&&budgetRules.length>0&&(
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    {budgetRules.map(rule=>{
                      ...
                      const bc=barColor(pct,rule.alert_pct);
                      const remaining=Math.max(rule.limit_value-spent,0);
                      const triggered=pct>=rule.alert_pct;
                      const accName=metaAccounts.find(a=>a.id===rule.account_id)?.name||"Todas as contas";
                      return(
                        <div key={rule.id} className="card" style={{background:T.card,borderRadius:14,border:`1px solid ${triggered?(pct>=100?"rgba(239,68,68,.3)":"rgba(245,158,11,.3)"):T.border}`,padding:"20px 24px"}}>
                          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,gap:12,flexWrap:"wrap"}}>
                            <div>
                              ...
                              <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                                <span style={{fontSize:11,color:T.mute}}>{accName}</span>
                                <span style={{fontSize:10,background:"rgba(255,255,255,.06)",borderRadius:5,padding:"2px 8px",color:T.mute}}>{periodLabel[rule.period]}</span>
                              </div>
                            </div>
                            <div style={{display:"flex",gap:6}}>
                          ...
                          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:10,marginBottom:16}}>
                            {[
                              {l:"Limite",v:fmtMoney(rule.limit_value,rule.currency),c:T.sub},
                              {l:"Gasto atual",v:fmtMoney(spent,rule.currency),c:bc},
                              {l:"Restante",v:fmtMoney(remaining,rule.currency),c:remaining<rule.limit_value*0.1?T.err:T.ok},
                              {l:"Alerta em",v:fmtMoney(rule.limit_value*(rule.alert_pct/100),rule.currency),c:T.warn},
                            ].map((m,i)=>(
                              <div key={i} style={{background:"rgba(255,255,255,.03)",borderRadius:9,border:`1px solid ${T.border}`,padding:"10px 14px"}}>
                                <div style={{fontSize:9,color:T.mute,letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>{m.l}</div>
                                <div style={{fontFamily:T.mono,fontSize:14,fontWeight:500,color:m.c}}>{m.v}</div>
                              </div>
                            ))}
                          </div>
                          ...
                          <div>
                            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                              <span style={{fontSize:11,color:T.sub}}>Progresso do período</span>
                              <span style={{fontSize:12,fontFamily:T.mono,fontWeight:600,color:bc}}>{pct}%</span>
                            </div>
                            ...
                            <div style={{display:"flex",justifyContent:"space-between",marginTop:5}}>
                              <span style={{fontSize:10,color:T.mute}}>R$ 0</span>
                              <span style={{fontSize:10,color:T.warn,position:"relative",left:`${rule.alert_pct-50}%`}}>▲ {rule.alert_pct}%</span>
                              <span style={{fontSize:10,color:T.mute}}>{fmtMoney(rule.limit_value,rule.currency)}</span>
                            </div>
                          </div>

                          {/* Editable alert threshold */}
                          <div style={{marginTop:14,paddingTop:12,borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                            <span style={{fontSize:11,color:T.mute}}>Alertar em:</span>
                            <div style={{display:"flex",gap:5}}>
                              {[60,70,80,90,95].map(p=>(
                                <button key={p} onClick={()=>updateBudgetRule(rule.id,"alert_pct",p)} style={{padding:"4px 10px",borderRadius:7,border:`1px solid ${rule.alert_pct===p?T.warn+"88":T.border}`,background:rule.alert_pct===p?"rgba(245,158,11,.1)":"transparent",color:rule.alert_pct===p?T.warn:T.mute,fontSize:11,fontFamily:T.font,cursor:"pointer"}}>
                                  {p}%
                                </button>
                              ))}
                            </div>
                            {!liveMetrics&&<span style={{fontSize:11,color:T.mute,marginLeft:"auto"}}>⚡ Conecte o BM para ver o gasto real</span>}
                            {liveMetrics&&<span style={{fontSize:11,color:T.ok,marginLeft:"auto"}}>✅ Gasto em tempo real via Meta API</span>}
                          </div>
                        </div>
                      );
                    })}
              ...
              <div>
                ...
                {waTab==="campaigns"&&(
                  <div>
                    {campsLoading?(
                      <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"60px 0",gap:10,color:T.mute,fontSize:13}}>
                        <span style={{width:16,height:16,border:`2px solid rgba(37,211,102,.2)`,borderTop:`2px solid ${WA}`,borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}></span>
